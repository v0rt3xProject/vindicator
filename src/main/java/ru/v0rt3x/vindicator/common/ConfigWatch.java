...
public class ConfigWatch implements Runnable {
    ...
    public void run() {
        try {
            Path configDir = VindicatorCore.getInstance().config().getConfigDir().toPath();
            Path configFile = VindicatorCore.getInstance().config().getConfigFile().toPath();

            logger.info("Watchdog started");
            try (final WatchService watchService = FileSystems.getDefault().newWatchService()) {
                final WatchKey watchKey = configDir.register(watchService, StandardWatchEventKinds.ENTRY_MODIFY);
                while (VindicatorCore.getInstance().isRunning()) {
                    final WatchKey wk = watchService.take();
                    for (WatchEvent<?> event : wk.pollEvents()) {
                        final Path changed = (Path) event.context();
                        if (changed.equals(configFile)) {
                            logger.info("Config file changed. Reloading...");
                            VindicatorCore.getInstance().config().readConfig();
                        }
                    }
